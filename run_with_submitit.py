...
def parse_args():
    ...
    parser = argparse.ArgumentParser("Submitit for iFormer", parents=[classification_parser])
    parser.add_argument("--ngpus", default=8, type=int, help="Number of gpus to request on each node")
    parser.add_argument("--nodes", default=2, type=int, help="Number of nodes to request")
    parser.add_argument("--timeout", default=72, type=int, help="Duration of the job, in hours")
    parser.add_argument("--job_name", default="iformer", type=str, help="Job name")
...
def get_shared_folder() -> Path:
    ...
    if Path("/checkpoint/").is_dir():
        p = Path(f"/checkpoint/{user}/iformer")
